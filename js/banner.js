...
function loads()
{
	...
	var img_num = $(".banner_img").children("li").length;//获取img标签的总数量
 	banner_max = img_num;
	...
	{
		numbers_span += '<span class="num'+i+'" onclick="manual_replace_banner('+i+')">'+'</span>'; //循环赋值到字符串
	}
...
function banner_number()
{
	for(var i=1; i<=banner_max; i++)
	{
		...
		if(banners == i)
		{
			...
			num_object.css("backgroundColor" , "#164695");
		}
		else
		{
			num_object.css("color" , "#164695");
