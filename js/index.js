...
$(document).ready(function () {
    ...
    EarlyWarningFun()
    LowerInterChangeFun()
    setInterval(LowerInterChangeFun, 1000 * 60 * 5)
})

/**@EarlyWarningFun :预警方法**/
function EarlyWarningFun() {
    let para = ''
    for (let i = 0; i < EmergencyListcopy.length; i++) {
        para += `<div class="Emergencyitem">
			<img  class="Emergencyicon" src=\images/Emergency/${EmergencyListcopy[i].imgs}.png\ >
				<p class="name">${EmergencyListcopy[i].name}</p>
		</div>`
    }
    document.getElementById("EmergencyId").innerHTML = para
    para = document.querySelectorAll("#EmergencyId .Emergencyitem")
    for (let i = 0; i < para.length; i++) {
        para[i].onclick = function () {
            switch (document.querySelectorAll("#EmergencyId .name")[i].innerHTML) {
                case '下立交':
                    break;
            }
        }
...
/**@LowerInterChangeFun :下立交预警方法**/
function LowerInterChangeFun() {
    let para = {
        url: InterChange_URL + '/sh/BigScreen/getJiShuiAlarm',
        async: true,
        type: 'get',
        dataType: 'JSON',
    }
    ajaxPromise(para).then(res => {
        ...
        list = res.filter(item => {
            let how = new Date(item.happenTime);
            let today = new Date();
            return parseInt((today.getTime() - how.getTime()) / 1000 / 60 / 60) < 1000
        })
        if (list.length > 0) {
            console.log(list,'++++++++++++++++++++++++++++++++=')
            EmergencyListcopy.map(item => {
                if (item.name == '下立交') {
                    item.imgs = '下立交yellow'
                }
            })
